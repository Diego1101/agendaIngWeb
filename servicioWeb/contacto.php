        ...
        switch($_REQUEST['op']){
            ...
            case 'detalle':
                if (!empty($_REQUEST['id'])) {
                    ...
                    include_once 'conexion.php';
                    if ($conn = mysqli_connect($server, $dbuser, $dbpass, $bd)) {
                        $renglon = mysqli_query($conn, "SELECT A.USU_CVE ID, CONCAT(A.USU_NOM,' ',A.USU_AP) NOMBRE, B.ROL_NOM ROL, A.USU_ROL USUROL, A.USU_TEL TEL, A.USU_EMAIL EMAIL, A.USU_USUARIO USUARIO, A.USU_CAR CARRERA, A.USU_SEM SEMESTRE, A.USU_GRU GRUPO, A.USU_NOM NOM, A.USU_AP AP 
